...
class NUTS3Filter(admin.SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        ...
        if val:
            ...
            results = queryset.filter(location__geometry__intersects=nuts3.geometry)
        else:
            results = queryset
...
class ArealFieldAdmin(nested_admin.NestedModelAdmin):
    ...
    def get_place(self, obj):
        if hasattr(obj.location, "address"):
            return obj.location.address.city
        else:
            return ", ".join([l.__str__() for l in Nuts3.objects.filter(geometry__intersects=obj.location.geometry)])

    def get_search_results(self, request, queryset, search_term):
        """Add NUTS3 (by name) search and area size search (using `<>` operator)
        """

        result, use_distinct = super(ArealFieldAdmin, self).get_search_results(request, queryset, search_term)
        if search_term:
    ...
    def _search_lay1_nuts3_by_name(self, queryset, search_term):
        ...
        for cls in (Lau1, Nuts3):
            ...
            for o in objs:
                objects = queryset.filter(location__geometry__intersects=o.geometry)
                filtered |= objects
        return filtered
    ...
    def _search_area(self, queryset, search_term):
        ...
        if search_term.find("<>") > -1:
            ...
            filtered = queryset.filter(
                    areal__area__total__gte=area_min, areal__area__total__lte=area_max)
        return filtered
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context['objects_data'] = json.dumps(self.as_geojson(filtered_query_set))
        ...
        for obj in queryset:
            if hasattr(obj, "location"):
                feature = {
                    ...
                    "properties": {
                        "title": obj.title,
                        "object_url": reverse('admin:{}_{}_change'.format(obj._meta.app_label,
                            obj._meta.model_name), args=(obj.pk,)),
                        "size": self.size(obj),
                    },
                    "geometry": json.loads(obj.location.geometry.centroid.json),
                    "id": obj.pk
                }
