...
class Command(BaseCommand):
    ...
    def import_addresses(self, address_file):
            ...
            if not address_file:
                ...
                response = requests.get('http://vdp.cuzk.cz/vymenny_format/csv/20191231_OB_ADR_csv.zip', stream=True)
        ...
        with open(f, encoding="windows-1250") as csv_f:
            ...
            for row in reader:
                ...
                (kod_adm, kod_obec, obec,
                    momc, nazev_momc, mop, nazev_mop, cast_obce_kod, cast_obce,
                    ulice_kod, ulice, so, cislo_domovni, cislo_orientacni,
                    znak_c_orientacniho,
                    psc, y, x, od) = row

                if second:
                    second = False
                    obec_objs = City.objects.filter(code = int(kod_obec))
                    if obec_objs:
                        obec_obj = obec_objs[0]
                    else:
